...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Plot N0009 / N0008 response'
    )
    ...
    for order in orders:
        ...
        np.savetxt(outfile,
                   np.transpose([
                       data['N0008'][order]['e_garf'],
                       data['N0008'][order]['garf'],
                       data['N0009'][order]['garf'],
                       data['N0008'][order]['rmf'],
                       data['N0009'][order]['rmf']]),
                   fmt=['%6g', '%4g', '%4g', '%4g', '%4g'],
                   header='energy\tN0008_garf\tN0009_garf\tN0008_rmf\tN0009_rmf',
                   delimiter='\t')
    ...
    for order in orders:
        wav = data['N0008'][order]['lambda']
        ratio = data['N0009'][order]['response'] / data['N0008'][order]['response']

        plt.plot(wav, ratio, colors[order], label=labels[order])
        
    plt.legend(loc='lower right', fontsize=10)
    plt.ylabel(r'$\textrm{N0009 / N0008 Response Ratio}$')
